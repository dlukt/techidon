package de.icod.techidon.updater;
...
public class GithubSelfUpdaterImpl extends GithubSelfUpdater{
	...
	private void actuallyCheckForUpdates(){
		Request req=new Request.Builder()
				.url("https://api.github.com/repos/deicod/techidon-nightly/releases")
				.build();
			...
			for (JsonElement jsonElement : arr) {
				...
				if(!tag.substring(tag.indexOf("-") + 1).equals(BuildConfig.VERSION_NAME.substring(BuildConfig.VERSION_NAME.lastIndexOf("@") + 1))){
					...
					for(JsonElement el:obj.getAsJsonArray("assets")){
						...
						if("techidon-nightly.apk".equals(asset.get("name").getAsString()) && "application/vnd.android.package-archive".equals(asset.get("content_type").getAsString()) && "uploaded".equals(asset.get("state").getAsString())){
