...
import de.icod.techidon.api.requests.announcements.DismissAnnouncement;
import de.icod.techidon.api.requests.statuses.CreateStatus;
import de.icod.techidon.api.requests.statuses.GetStatusSourceText;
...
import de.icod.techidon.fragments.ListsFragment;
...
public class HeaderStatusDisplayItem extends StatusDisplayItem{
	...
	public HeaderStatusDisplayItem(String parentID, Account user, Instant createdAt, BaseStatusListFragment parentFragment, String accountID, Status status, CharSequence extraText, Notification notification, ScheduledStatus scheduledStatus){
		...
		if(status!=null){
			...
			for (de.icod.techidon.model.Attachment m : status.mediaAttachments) {
				if (m.type.isImage()) {
