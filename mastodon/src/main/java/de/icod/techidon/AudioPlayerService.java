...
public class AudioPlayerService extends Service{
	...
	private static final String ACTION_PLAY_PAUSE="de.icod.techidon.AUDIO_PLAY_PAUSE";
	private static final String ACTION_STOP="de.icod.techidon.AUDIO_STOP";
...
//		registerReceiver(receiver, new IntentFilter(Intent.ACTION_MEDIA_BUTTON));
		if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.TIRAMISU){
			registerReceiver(receiver, new IntentFilter(AudioManager.ACTION_AUDIO_BECOMING_NOISY), RECEIVER_EXPORTED);
	...
	public int onStartCommand(Intent intent, int flags, int startId){
		...
		Parcelable attachmentParcelable;
		if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.TIRAMISU){
			statusParcelable=intent.getParcelableExtra("status", Parcelable.class);
			attachmentParcelable=intent.getParcelableExtra("attachment", Parcelable.class);
		}else{
			statusParcelable=intent.getParcelableExtra("status");
			attachmentParcelable=intent.getParcelableExtra("attachment");
		}
		status=Parcels.unwrap(statusParcelable);
		attachment=Parcels.unwrap(attachmentParcelable);
		...
		if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.O){
			AudioAttributes attrs=new AudioAttributes.Builder()
					...
					.build();
			audioFocusRequest=new AudioFocusRequest.Builder(audiofocus)
					.setAudioAttributes(attrs)
					.setOnAudioFocusChangeListener(audioFocusChangeListener)
					.build();
			audioManager.requestAudioFocus(audioFocusRequest);
		}else{
			audioManager.requestAudioFocus(audioFocusChangeListener, AudioManager.STREAM_MUSIC, audiofocus);
		}
