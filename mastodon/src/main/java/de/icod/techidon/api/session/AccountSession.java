package de.icod.techidon.api.session;
...
import de.icod.techidon.api.PushSubscriptionManager;
import de.icod.techidon.api.StatusInteractionController;
import de.icod.techidon.api.requests.accounts.GetPreferences;
import de.icod.techidon.api.requests.accounts.UpdateAccountCredentialsPreferences;
import de.icod.techidon.api.requests.markers.GetMarkers;
...
import de.icod.techidon.model.AltTextFilter;
...
public class AccountSession{
	...
	public <T> boolean filterStatusContainingObject(T object, Function<T, Status> extractor, FilterContext context, Account profile){
		...
		if(getLocalPreferences().serverSideFiltersSupported){
			// Techidon: this code path in CustomLocalTimelines makes the app crash, so this check is here
			if (s.filtered == null)
				return false;
