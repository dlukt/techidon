...
public abstract class BaseRecyclerFragment<T> extends LoaderFragment implements SwipeRefreshLayout.OnRefreshListener, ListImageLoaderWrapper.Listener, Preloader.Callback<T>{
	...
	public static class RetainedState extends ViewModel{
		...
		boolean dataLoading;
		boolean refreshing;
		boolean preloadingFailed;
		boolean refreshEnabled=true;
		boolean refreshAfterCreate;
	}
	...
	public void onCreate(Bundle savedInstanceState){
		...
		retainedState=new ViewModelProvider(this).get(RetainedState.class);
		if(retainedState.preloader==null){
			retainedState.preloader=preloader;
			retainedState.itemsPerPage=itemsPerPage;
			retainedState.listLayoutId=listLayoutId;
			retainedState.refreshEnabled=refreshEnabled;
		}else{
			@SuppressWarnings("unchecked")
			Preloader<T> restored=(Preloader<T>) retainedState.preloader;
			preloader=restored;
			itemsPerPage=retainedState.itemsPerPage;
			listLayoutId=retainedState.listLayoutId;
			refreshEnabled=retainedState.refreshEnabled;
			refreshAfterCreate=retainedState.refreshAfterCreate;
			preloadingFailed=retainedState.preloadingFailed;
	...
	private void syncRetainedState(){
		if(retainedState==null)
			return;
		retainedState.itemsPerPage=itemsPerPage;
		retainedState.listLayoutId=listLayoutId;
		retainedState.refreshEnabled=refreshEnabled;
		retainedState.refreshAfterCreate=refreshAfterCreate;
		retainedState.preloadingFailed=preloadingFailed;
		retainedState.loaded=loaded;
		retainedState.dataLoading=dataLoading;
		retainedState.refreshing=refreshing;
	}
}
